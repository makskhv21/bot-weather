...
const KeyboardOptions = {
    ...
    USE_REMIND: 'Нагадування',
};

const NotificationKeyboardOptions = {
    SET_NOTIFICATION: 'Налаштувати щоденне нагадування',
    REMOVE_NOTIFICATION: 'Видалити час нагадування',
...
bot.command('setnotification', (ctx) => {
    ctx.reply('Введіть час нагадування у форматі HH:mm (наприклад, 08:30):');
...
bot.command('about', (ctx) => {
    ...
    💬 Ви можете додавати міста, використовувати свою геолокацію для визначення погоди та налаштовувати щоденні нагадування.
    `;
...
bot.command('help', (ctx) => {
    ...
    - /setnotification — Налаштувати щоденне нагадування для міста
    - /remove_notification — Видалити час нагадування
    `;
...
bot.hears(NotificationKeyboardOptions.SET_NOTIFICATION, (ctx) => {
    ...
    if (users[userId]?.cities?.length > 0) {
        ctx.reply('Оберіть місто для налаштування нагадування:', getCityForNotificationKeyboard(userId));
    } else {
...
bot.hears(NotificationKeyboardOptions.REMOVE_NOTIFICATION, (ctx) => {
    ...
    if (cities.length > 0) {
        ctx.reply('Оберіть місто для видалення часу нагадування:', getCityForNotificationKeyboard(userId));
    } else {
        ctx.reply('❌ У вас ще немає міст. Спочатку додайте місто.');
    }
});
...
bot.on('message', async (ctx) => {
    ...
    if (ctx.session.stage === 'set_notification_time') {
        ...
        if (timeRegex.test(time)) {
            ...
            ctx.reply(`Час нагадування для міста "${city}" встановлено на ${time}.`, getNotificationKeyboard());
            ctx.session.stage = undefined;
        } else {
    ...
    if (ctx.message.text === NotificationKeyboardOptions.REMOVE_NOTIFICATION) {
        if (users[userId]?.notificationTime) {
            ...
            ctx.reply('Час нагадування видалено.', getNotificationKeyboard());
        } else {
            ctx.reply('Немає налаштованого часу нагадування.', getNotificationKeyboard());
        }
    }
});
...
bot.action(/^set_notification_(.+)$/, (ctx) => {
    ...
    ctx.reply(`Вибрано місто: ${city}. Тепер введіть час нагадування у форматі HH:mm (наприклад, 08:30):`);
    ctx.session.stage = 'set_notification_time';
});
...
bot.action(/^remove_notification_(.+)$/, (ctx) => {
    ...
    if (users[userId]?.notifications && users[userId].notifications[city]) {
        ...
        ctx.reply(`Час нагадування для міста "${city}" видалено.`, getNotificationKeyboard());
    } else {
        ctx.reply('❌ Немає налаштованого часу нагадування для цього міста.');
    }
});
